...
object StackBlur {
    ...
    fun blur(original: Bitmap, radius: Float, blurProcess: BlurProcess.ThreadsController): Bitmap? {
        val w = original.width
        val h = original.height
        val currentPixels = IntArray(w * h)
        original.getPixels(currentPixels, 0, w, 0, 0, w, h)
        val cores: Int = blurProcess.threads
        ...
        try {
            blurProcess.executor.invokeAll(horizontal)
        } catch (e: InterruptedException) {
            return null
        }
        try {
            blurProcess.executor.invokeAll(vertical)
        } catch (e: InterruptedException) {
            return null
        }
